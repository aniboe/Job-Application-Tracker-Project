...
function ApplicationPopup({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800/80 flex items-start justify-between gap-4">
        <div>
      ...
      {/* Details List */}
      <div className="px-6 py-4 divide-y divide-zinc-100 dark:divide-zinc-800/70">
        {/* Company Name */}
        <div className="py-3 flex items-center justify-between text-sm">
          <span className="text-zinc-500 dark:text-zinc-400 font-normal">Company</span>
        ...
        {/* Salary */}
        <div className="py-3 flex items-center justify-between text-sm">
          <span className="text-zinc-500 dark:text-zinc-400 font-normal">Salary Range</span>
          <span className="text-zinc-800 dark:text-zinc-200 font-medium">
        ...
        {/* Status */}
        <div className="py-3 flex items-center justify-between text-sm">
          <span className="text-zinc-500 dark:text-zinc-400 font-normal">Status</span>
          <span
        ...
        {selectedApplication?.companyLink && (
          <div className="py-3 flex items-center justify-between text-sm">
            <span className="text-zinc-500 dark:text-zinc-400 font-normal">Website</span>
            <a
              href={selectedApplication.companyLink}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1 text-zinc-800 dark:text-zinc-200 hover:text-zinc-950 dark:hover:text-white font-medium underline underline-offset-4 decoration-zinc-300 dark:decoration-zinc-700 hover:decoration-zinc-800 dark:hover:decoration-zinc-300 transition-colors max-w-55 truncate"
            >
              <span className="truncate">{selectedApplication.companyLink.replace(/^https?:\/\//, '')}</span>
              <LuExternalLink size={14} className="shrink-0 text-zinc-400 dark:text-zinc-500" />
            </a>
          </div>
        )}
      </div>
      ...
      {/* Footer Actions */}
      <div className="px-6 py-4 bg-zinc-50/75 dark:bg-zinc-900/60 border-t border-zinc-100 dark:border-zinc-800/80 flex items-center justify-between transition-colors">
        <button
          type="button"
          onClick={() => {
            setShowDeleteWarning(true);
          }}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-lg transition-colors cursor-pointer"
        >
          <LuTrash2 size={15} />
          <span>Delete</span>
        </button>
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setSetshowApplication(false)}
            className="px-3.5 py-1.5 text-xs font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 hover:bg-zinc-200/60 dark:hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer"
          >
            Close
          </button>
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-medium text-white dark:text-zinc-950 bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-zinc-200 rounded-lg transition-colors cursor-pointer shadow-xs"
          >
            <LuPencil size={13} />
            <span>Edit</span>
          </button>
